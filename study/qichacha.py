...
class EnterpriseInfoSpider:
    ...
    def getFirmIdDoms(self,catalogPageCode):
        soup = BeautifulSoup(catalogPageCode,"html.parser")
        return soup.select("#searchlist .m_srchList a")
        #return soup.select("#searchlist .table-search-list .tp2 a")

    #爬虫开始
    def start(self):
        keyword = raw_input("请输入关键字:").decode("utf-8",'ignore')
        #keyword = raw_input("请输入关键字:").decode("utf-8",'ignore')
       # keyword = keyword .decode('utf-8', 'ignore')
        while keyword != "end":
            ...
            if totalPage == -1:
                # 请求下一轮查询的关键字
                keyword = raw_input("爬取结束,请输入关键字：").decode("utf-8","ignore")
                continue
    ...
    def writeDetailsToExcel(self,detailsPageCode,companyname,phone,email):
        ...
        for detailDom in detailDoms:
            ...
            while col < len(self.fields):
                # 找到匹配的那列字段
                if detailName == self.fields[col].decode('gbk').encode("utf-8"):
